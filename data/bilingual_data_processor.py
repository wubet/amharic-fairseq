...
class BilingualDataPreprocessor:
    ...
    def save_am_tokenizer(self, filepath):
        """
        Saves the Amharic tokenizer to a file.

        Parameters:
        - filepath: Path to the file where the tokenizer should be saved.
        """
        # Construct the new path by appending 'am_tokenizer.json' to the parent directory
        new_path = os.path.join(filepath, 'am_tokenizer.json')
        self.am_tokenizer.save(new_path)

    def save_en_tokenizer(self, filepath):
        """
        Saves the English tokenizer to a file.

        Parameters:
        - filepath: Path to the file where the tokenizer should be saved.
        """
        # Construct the new path by appending 'am_tokenizer.json' to the parent directory
        new_path = os.path.join(filepath, 'en_tokenizer.json')
        self.en_tokenizer.save(new_path)
    ...
    def preprocess_data(self, en_file, am_file, imp, pre_fix):
        ...
        if self.use_bert_tokenizer:
            ...
            if pre_fix.endswith("train.en-am"):
                path_directory = os.path.dirname(pre_fix)
                self.save_am_tokenizer(path_directory)
                self.save_en_tokenizer(path_directory)
